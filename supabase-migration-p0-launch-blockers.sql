...
BEGIN
  ...
  SELECT count(*) INTO v_failures
   ...
   WHERE user_id = uid AND succeeded = false
  ...
  IF v_group.id IS NULL
    ...
    INSERT INTO public.join_code_attempts(user_id, succeeded) VALUES (uid, false);
  ...
  IF v_member.id IS NULL THEN
    ...
    INSERT INTO public.group_members(
      ...
      v_member.id, v_group.id, uid, COALESCE(NULLIF(trim(p_display_name), ''), 'Member'),
      'member', 'connected', uid, v_now
    );
    was_already_connected := false;
  ELSE
    was_already_connected := v_member.status = 'connected';
    UPDATE public.group_members
       SET status = 'connected', joined_at = COALESCE(joined_at, v_now)
     WHERE id = v_member.id;
  END IF;
  INSERT INTO public.join_code_attempts(user_id, succeeded) VALUES (uid, true);
...
BEGIN
  ...
  SELECT * INTO v_member FROM public.group_members
   WHERE group_id = v_invite.group_id AND profile_id = uid LIMIT 1;
  IF v_member.id IS NULL AND v_invite.linked_member_id IS NOT NULL THEN
    SELECT * INTO v_member FROM public.group_members
     WHERE id = v_invite.linked_member_id AND group_id = v_invite.group_id
     FOR UPDATE;
  END IF;
  ...
  IF v_member.id IS NULL THEN
    ...
    INSERT INTO public.group_members(
      ...
      v_member.id, v_invite.group_id, uid, COALESCE(NULLIF(trim(p_display_name), ''), 'Member'),
      'member', 'connected', v_invite.created_by, v_now
    );
    was_already_connected := false;
  ELSE
    was_already_connected := v_member.status = 'connected' AND v_member.profile_id = uid;
    UPDATE public.group_members
       SET profile_id = uid, status = 'connected', joined_at = COALESCE(joined_at, v_now)
     WHERE id = v_member.id;
  END IF;

  UPDATE public.group_invites
     SET accepted_by = uid, accepted_at = COALESCE(accepted_at, v_now)
