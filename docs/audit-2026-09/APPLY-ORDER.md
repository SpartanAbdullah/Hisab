...
### Two things worth flagging while this table exists

- **`apply-order.txt` lists `supabase-migration-p1-profile-lang.sql` twice** (once in its correct P1 position, once more a few lines later, immediately before the P2 block comment). Both lines name the same file, so applying the corpus top-to-bottom just re-runs an idempotent migration a second time — harmless, but it is drift in the ordering file itself and worth a one-line fix next time anyone touches it.
