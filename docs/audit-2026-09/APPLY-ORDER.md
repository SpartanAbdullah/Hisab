...
against the result — **618 assertions, all passing** as of 2026-09-03 (the
figure has grown with each suite: 183 → 248 → 331 → 357 → 449 → 578 → 618;
183 was 152 from the original trust-boundary suite
`docs/testing-the-trust-boundary.md` + 31 from `60-notification-maturity.sql`,
and the last two steps are `90-performance-hardening.sql` and
`92-function-grants.sql`). CI runs the same script on every push/PR to
...
- **`supabase/tests/apply-order.txt` remains the single canonical apply order** for the whole corpus — restated, not changed, by this update. It now lists **76 non-comment, non-blank lines**: 1 harness-only prelude (`supabase/tests/prelude-notifications-rls.sql`, never applied to production) + **75 production files** (`supabase-schema.sql` + 74 `supabase-migration-*.sql` files), which matches `ls supabase-migration-*.sql | wc -l` (74) + the base schema exactly. (Was 75 lines / 74 files until later on 2026-09-04, when `p3-account-deletion-balance-gate.sql` was added; 74 / 73 before `p3-currencies-iso4217.sql` the same day.) (Was 72 lines / 71 files earlier on 2026-09-03; `p3-rls-initplan-and-indexes.sql` and `p3-rpc-execute-grants.sql` were added the same day.) Do not hand-merge this table with `apply-order.txt`; if they ever disagree, `apply-order.txt` wins.
...
- **Added 2026-09-03 (late) — `supabase-migration-p3-rls-initplan-and-indexes.sql`, positioned immediately BEFORE `p3-invariant-monitoring.sql`.** Closes the Supabase **Performance Advisor** findings from the same production run (173 lints: 87 WARN `auth_rls_initplan`, 20 WARN `multiple_permissive_policies`, 1 WARN `duplicate_index`, 27 INFO `unindexed_foreign_keys`, 36 INFO `unused_index`, 2 INFO `no_primary_key`). §2 is a generic `DO` sweep over `pg_policy` that rewrites every bare `auth.uid()` / `auth.jwt()` / `auth.role()` / `current_setting()` in a USING or WITH CHECK clause to the `( SELECT … )` initplan form via `ALTER POLICY` (94 policies on the full corpus, 0 on a re-run; the branch's own files use the bare form 225 times, which is why this runs late rather than being fixed file by file). §1 consolidates `split_groups`' FOR ALL policy into INSERT/UPDATE/DELETE twins with identical semantics (the FOR ALL predicate is a strict subset of the SELECT one), guarded to abort if the SELECT policy is absent; the 15 `group_settlements` overlaps were already closed by `audit-p0-group-ledger-integrity.sql`. §3 drops `idx_group_events_group_created` (`supabase-schema.sql`) in favour of `idx_gevents_group_created` (`performance-indexes.sql`). §4 adds 14 covering indexes on FKs that `supabaseDb.ts` or an RPC actually filters on (`CREATE INDEX IF NOT EXISTS`, no `CONCURRENTLY` — Studio applies the file in one transaction; split §4 out if a table is large); the 15 `*_by` audit columns are deliberately skipped. §5 gives `join_code_attempts`, `phone_lookup_attempts` and the branch-new `code_lookup_attempts` (append-only rate-limit logs with no natural key) a `BIGINT GENERATED ALWAYS AS IDENTITY` primary key. The 36 `unused_index` INFOs are listed in the header as a 30-days-after-launch follow-up, not acted on. Docker-validated (`postgres:15`, full corpus, idempotent across 3 applies with a byte-identical `pg_policies` snapshot); `supabase/tests/tests/90-performance-hardening.sql` adds **31** assertions, 14 of them behavioural RLS walks (owner / connected member / ex-member / stranger × SELECT/INSERT/UPDATE/DELETE on `split_groups`) proving §1 changed nothing observable.
- **Added 2026-09-03 (late) — `supabase-migration-p3-rpc-execute-grants.sql`, and it is now the LAST line of `apply-order.txt`.** Closes the three Supabase **Security Advisor** findings from the production run recorded in [prod-verification-2026-09-03.md](prod-verification-2026-09-03.md) (`0 ERROR / 56 WARN / 2 INFO`): (a) 19 SECURITY DEFINER functions in `public` EXECUTE-able by `anon`, (b) 31 EXECUTE-able by `authenticated` including trigger/internal functions, (c) 5 functions with a mutable `search_path`. It is a **sweep over `pg_proc`**, not a list of hand-written grants: REVOKE EXECUTE FROM `PUBLIC`+`anon` on every SECURITY DEFINER function, REVOKE from `authenticated` on everything that is not one of the 56 `supabase.rpc()` names `grep` finds in `src/`, REVOKE both roles off every `tg_*` (plus `handle_new_user`, `enforce_group_expense_reconciliation_payer`, `hisaab_broadcast_money_change`, `touch_app_config_updated_at`), pin `search_path = public, extensions, pg_temp` on every function still lacking one, and `ALTER DEFAULT PRIVILEGES … REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC, anon` so the next `CREATE FUNCTION` is safe by construction. **Position:** after `p3-invariant-monitoring.sql` (previously last) — anything applied after a `pg_proc` sweep escapes it, and `p3-invariant-monitoring` contributes five of the eleven functions that needed a `search_path` pin. **Deliberate `anon` allowlist — exactly TWO:** `get_committee_witness` and `get_khata_view`, the two capability-URL public pages rendered above every gate in `src/App.tsx:820/832`. Nothing else. **Four functions keep `authenticated` without being client RPCs** and this is the part that is easy to get wrong: `is_current_profile_active` and `is_group_member` are referenced by 34 and 9 RLS policies, and Postgres privilege-checks a policy expression against the *querying* role — revoking either from `authenticated` turns every table read into `42501` (proven empirically in the harness; header §R1). `group_settlement_cap` and `group_member_net_balances` are SECURITY DEFINER helpers called from the SECURITY INVOKER triggers `tg_group_settlements_enforce_cap` and `tg_split_groups_guard_delete`, which run as the writing user (§R2). All four are denied to `anon`: the only table this app reads with no session is `app_config`, whose policy is a function-free `TO anon, authenticated USING (true)` (`p1-app-config.sql:166`) — so `is_group_member` loses the `anon` EXECUTE it had, closing a membership oracle. Docker-validated in the full corpus run (`postgres:15`, all 74 `apply-order.txt` lines, **618 assertions / 0 failed**); `supabase/tests/tests/92-function-grants.sql` adds **40** of those, incl. the behavioural proof that revoked trigger functions still *fire* (Postgres checks EXECUTE at `CREATE TRIGGER`, never at fire time) and that `anon` can still call both public-page RPCs. Advisor INFO item (d) — `app_push_config` / `phone_lookup_attempts` RLS-on-with-zero-policies — is **confirmed by design and left alone**; note that `audit-p0-consent-guards.sql:302` turns the second one into an explicit `USING (false)` deny-all, so after the branch that INFO disappears rather than the table becoming client-readable.
- **Added 2026-09-04 — `supabase-migration-p3-currencies-iso4217.sql`, positioned between `p3-rls-initplan-and-indexes.sql` and `p3-invariant-monitoring.sql`.** The founder decision of the same date: Hisaab accepts **every active ISO 4217 currency**, not just the eight the client ships. It creates `public.currencies` (157 seeded rows — `code CHAR(3)` PK, `name_en`, `minor_units` ∈ {0,2,3}, `is_active`, `sort_order`; RLS on, one function-free `TO anon, authenticated USING (true)` SELECT policy, `REVOKE ALL` then `GRANT SELECT` so the Supabase default `GRANT ALL ON TABLES` cannot leave a write behind) and **replaces all 16 currency whitelist CHECKs with FOREIGN KEYs** into `currencies(code)`, `ON UPDATE CASCADE ON DELETE RESTRICT` — the 14 `<table>_<column>_supported` generated by `p1-money-bounds.sql` §2a plus `ltr_currency_supported` / `lsr_currency_supported` from `audit-p0-currencies.sql`. Each FK is added `NOT VALID` and `VALIDATE`d inside an exception handler (the p1-money-bounds §1 pattern), so **the apply cannot abort on legacy data**; each CHECK is dropped by name with a NOTICE-not-error fallback, plus a sweep for any leftover CHECK on the same column that still enumerates a currency literal. The eight shipped currencies are seeded, so this is a **strict widening** and cannot invalidate a row — measured, not assumed, by the read-only `supabase-preflight-currencies-2026-09-04.sql` (repo root; not named `supabase-migration-*` on purpose — `db-tests.yml` globs that prefix, `tasks/lessons.md` 2026-09-03). **Position rationale:** it must follow `p1-money-bounds`/`audit-p0-currencies` (it drops their constraints by name); it sits after the initplan sweep so that file stays the last thing to touch a policy (this file's policy is `USING (true)`, so the sweep is a no-op on it either way); and it sits *before* `p3-rpc-execute-grants.sql` despite that file's "must be last" rule because **that sweep is over `pg_proc` only** — its §4 is `ALTER DEFAULT PRIVILEGES … ON FUNCTIONS` (`:502-507`), there is no `ON TABLES` clause and no table GRANT/REVOKE anywhere in it — and this file creates zero functions, so nothing below it can alter `currencies`' grants. Docker-validated (`postgres:15`, full corpus, **651 assertions / 0 failed**, up from 618): `supabase/tests/tests/93-currencies-iso4217.sql` adds **33**; `40-money-integrity.sql`'s "USD is refused" assertion is flipped to "USD is accepted" (same assertion count) and its XXX negative now matches the FK name; `90-performance-hardening.sql`'s uncovered-FK census excludes the 16 `*_fk_currency` constraints with the reason inline (currencies are never DELETEd or key-UPDATEd, so the parent-side scan the index would serve never happens). **Idempotence proved by applying the file three times** in a row against the fully-migrated database (exit 0 each time, V1-V6 identical, suite still 651/0). The rollback block at the bottom is validated in both branches: with an out-of-eight row present it restores 15 CHECKs, keeps the one FK it cannot replace, and refuses to drop the table (guarded — a bare `DROP TABLE` there would abort and undo the restore); with no such row it restores all 16 and drops `currencies`. Model, minor-units rule, and the add/disable runbook: [docs/currencies.md](../currencies.md).
- **Added 2026-09-04 (later) — `supabase-migration-p3-account-deletion-balance-gate.sql`, positioned between `p3-currencies-iso4217.sql` and `p3-invariant-monitoring.sql`.** Founder decision D1 of the same date (P0-REMEDIATION.md §6.1): account deletion **refuses until settled, mirroring `leave_group`**. It re-creates `delete_current_user()` — the `p2-trust-safety.sql` §8.3 body verbatim — with one added guard between the owner guard and the departure loop: for every group where the caller is `'connected'` **and** another profile-linked member is still connected, `group_member_net_balance(group_id, member_id)` (audit-p0-group-concurrency; same arithmetic as `leave_group` and both group guards) is checked against the 0.01 tolerance, and any hit raises `UNSETTLED_GROUP_BALANCES` with `DETAIL = 'Flatmates: owes AED 20.00; …'`. The counterparty predicate is deliberate — with nobody left to settle with, a refusal would be a permanent trap — and `leave_group`'s unreconciled-expense gate is deliberately **not** mirrored (reconciliation is the payer's private bookkeeping, which ceases with the account). Owner guard first, so an owner hears about the groups they run before the balances that transferring may change. Touches no table, policy or data, so it sits with the late function replacements; it must precede the `pg_proc` grants sweep, which re-pins it. Docker-validated (`postgres:15`, full corpus, **656 assertions / 0 failed**, up from 651): `50-lifecycle-and-config.sql` adds **5** — the debtor's refusal, the account and membership left intact, the exact DETAIL text (via `GET STACKED DIAGNOSTICS`), the guard order (the creditor who also owns the group still sees `OWNED_GROUPS_WITH_MEMBERS`), and the post-settlement zero net that lets the existing "plain member CAN delete" assertion keep passing. Client: `SettingsPage.readUnsettledBalancesBlocker` + `del_account_unsettled_*` copy; the listing's deletion line is now "hand over any group you run and settle what you owe first".
