...
- **`supabase/tests/apply-order.txt` remains the single canonical apply order** for the whole corpus — restated, not changed, by this update. It now lists **74 non-comment, non-blank lines**: 1 harness-only prelude (`supabase/tests/prelude-notifications-rls.sql`, never applied to production) + **73 production files** (`supabase-schema.sql` + 72 `supabase-migration-*.sql` files), which matches `ls supabase-migration-*.sql | wc -l` (72) + the base schema exactly. (Was 72 lines / 71 files earlier on 2026-09-03; `p3-rls-initplan-and-indexes.sql` and `p3-rpc-execute-grants.sql` were added the same day.) Do not hand-merge this table with `apply-order.txt`; if they ever disagree, `apply-order.txt` wins.
