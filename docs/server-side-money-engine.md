...
Server truth afterwards: **the money left the wallet and arrived nowhere.** No transaction row, no marker, no repair queue — there is no offline write queue (the inert outbox scaffold was deleted on 2026-09-04, decision D5 in `docs/offline-story.md`). The user sees a generic error and, once a refetch lands, a balance that is simply smaller. Recovery is the `adjustment` type, by hand, if they notice.
...
- **Offline.** An atomic RPC still needs a connection. M1's other half — persisting pending work and replaying it — was decided on 2026-09-04 (D5, Option A: the inert outbox scaffold is deleted and the app is explicitly online-required for writes; a narrow single-row replay queue stays a telemetry-gated backlog item — `docs/offline-story.md`), so the honest answer remains "Hisaab needs a network to record money".
...
8. **Offline.** An atomic RPC still needs a connection. M1's other half — persisting pending work and replaying it — was decided on 2026-09-04 (D5, Option A: scaffold deleted, app online-required for writes; Option B a telemetry-gated backlog item — `docs/offline-story.md`), so the honest answer remains "Hisaab needs a network to record money".
