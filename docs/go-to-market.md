...
|---|---|---|---|
| 1 | 8 supported currencies (PKR, AED, PHP, SAR, QAR, OMR, KWD, BHD), no USD/EUR/GBP | **TRUE — already corrected** in `docs/play-store-listing.md` on 2026-09-02 | None — already live in the listing doc. |
...
| 3 | "Offline-first — syncs when you're back online" | **FALSE / not claimed** — correctly removed. The write outbox is a disabled scaffold (`outboxRunner.ts:26-29`, all handlers throw) | Do not reintroduce until the outbox is actually built and integration-tested (roadmap item M1, effort L). This is not a near-term claim. |
