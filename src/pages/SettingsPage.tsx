...
export function SettingsPage() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      toast.show({ type: 'success', title: t('settings_export'), subtitle: `hisaab_backup_${date}.json` });
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (!confirm(t('settings_import_warn'))) { e.target.value = ''; return; }
    try {
      ...
      if (result.success) {
        ...
        toast.show({ type: 'error', title: t('settings_import_fail'), subtitle: result.message });
      }
    } catch { toast.show({ type: 'error', title: t('settings_import_fail') }); }
    e.target.value = '';
  };

  const handleSetPin = async () => {
    if (pin1.length !== 4 || pin2.length !== 4) return;
    if (pin1 !== pin2) { toast.show({ type: 'error', title: t('pin_mismatch') }); return; }
    await setPin(pin1);
    toast.show({ type: 'success', title: t('pin_set_success') });
    setShowPinSetup(false); setPin1(''); setPin2('');
  };
  ...
  const rowClass = "flex items-center gap-3 px-4 py-3.5 active:bg-slate-50 transition-colors";
    ...
    <div className="pb-28 bg-mesh min-h-dvh">
      ...
      <div className="px-5 pt-5 space-y-4">
        ...
        <div className={sectionClass}>
          <button onClick={() => setShowProfile(!showProfile)} className={rowClass + ' w-full text-left'}>
            <div className="w-9 h-9 rounded-xl bg-indigo-50 flex items-center justify-center"><User size={16} className="text-indigo-500" /></div>
            <div className="flex-1">
              <p className="text-[13px] font-semibold text-slate-700">{t('settings_my_account')}</p>
              <p className="text-[11px] text-slate-400">{userName || t('settings_my_account_desc')}</p>
            </div>
            <ChevronRight size={16} className={`text-slate-300 transition-transform ${showProfile ? 'rotate-90' : ''}`} />
          </button>
          {showProfile && (
        ...
        {/* Language */}
        <div className={sectionClass}>
          <button onClick={() => setLang(lang === 'ur' ? 'en' : 'ur')} className={rowClass + ' w-full text-left'}>
            <div className="w-9 h-9 rounded-xl bg-blue-50 flex items-center justify-center"><Globe size={16} className="text-blue-500" /></div>
            <div className="flex-1">
              <p className="text-[13px] font-semibold text-slate-700">{t('settings_language')}</p>
              <p className="text-[11px] text-slate-400">{lang === 'ur' ? 'Roman Urdu' : 'English'}</p>
            </div>
        ...
        <div className={sectionClass}>
          <div className={rowClass}>
            <div className="w-9 h-9 rounded-xl bg-purple-50 flex items-center justify-center"><Smartphone size={16} className="text-purple-500" /></div>
            <div className="flex-1">
              <p className="text-[13px] font-semibold text-slate-700">{t('settings_app_mode')}</p>
              <p className="text-[11px] text-slate-400">{t('settings_mode_current')}: {mode === 'splits_only' ? t('mode_splits_title') : t('mode_full_title')}</p>
            </div>
          </div>
          <div className="p-4 flex gap-2">
            ...
            <button onClick={() => setMode('full_tracker')}
        ...
        <div className={sectionClass}>
          <div className={rowClass}>
            <div className="w-9 h-9 rounded-xl bg-amber-50 flex items-center justify-center"><Shield size={16} className="text-amber-500" /></div>
            <div className="flex-1">
              <p className="text-[13px] font-semibold text-slate-700">{t('settings_security')}</p>
              <p className="text-[11px] text-slate-400">{t('settings_pin_desc')}</p>
            </div>
          </div>
          {showPinSetup ? (
            <div className="p-4 space-y-3">
              <input type="password" inputMode="numeric" maxLength={4} placeholder={t('pin_set_title')} value={pin1} onChange={e => setPin1(e.target.value.replace(/\D/g, ''))}
                className="w-full border border-slate-200/60 rounded-xl px-4 py-3 text-center text-lg tracking-[0.5em] font-bold" />
              <input type="password" inputMode="numeric" maxLength={4} placeholder={t('pin_confirm')} value={pin2} onChange={e => setPin2(e.target.value.replace(/\D/g, ''))}
                className="w-full border border-slate-200/60 rounded-xl px-4 py-3 text-center text-lg tracking-[0.5em] font-bold" />
              <div className="flex gap-2">
                <button onClick={() => { setShowPinSetup(false); setPin1(''); setPin2(''); }} className="flex-1 py-2.5 rounded-xl bg-slate-100 text-slate-500 text-[12px] font-bold">Cancel</button>
                <button onClick={handleSetPin} disabled={pin1.length !== 4 || pin2.length !== 4}
                  className="flex-1 py-2.5 rounded-xl btn-gradient text-[12px] font-bold disabled:opacity-30">Save</button>
              </div>
            </div>
          ) : (
            <div className="p-4 flex gap-2">
              {hasPin ? (
                <>
                  <button onClick={() => setShowPinSetup(true)} className="flex-1 py-2.5 rounded-xl bg-slate-100 text-slate-600 text-[12px] font-bold flex items-center justify-center gap-1.5">
                    <Lock size={12} /> {t('settings_change_pin')}
                  </button>
                  <button onClick={handleRemovePin} className="flex-1 py-2.5 rounded-xl bg-red-50 text-red-500 text-[12px] font-bold flex items-center justify-center gap-1.5">
                    <Unlock size={12} /> {t('settings_remove_pin')}
                  </button>
                </>
              ) : (
                <button onClick={() => setShowPinSetup(true)} className="flex-1 py-2.5 rounded-xl btn-gradient text-[12px] font-bold flex items-center justify-center gap-1.5">
        ...
        <div className={sectionClass}>
          <button onClick={handleExport} disabled={exporting} className={rowClass + ' w-full text-left'}>
            <div className="w-9 h-9 rounded-xl bg-emerald-50 flex items-center justify-center"><Download size={16} className="text-emerald-500" /></div>
            <div className="flex-1">
              <p className="text-[13px] font-semibold text-slate-700">{t('settings_export')}</p>
              <p className="text-[11px] text-slate-400">{t('settings_export_desc')}</p>
            </div>
            <ChevronRight size={16} className="text-slate-300" />
          </button>
          <button onClick={() => fileRef.current?.click()} className={rowClass + ' w-full text-left'}>
            <div className="w-9 h-9 rounded-xl bg-blue-50 flex items-center justify-center"><Upload size={16} className="text-blue-500" /></div>
            <div className="flex-1">
              <p className="text-[13px] font-semibold text-slate-700">{t('settings_import')}</p>
              <p className="text-[11px] text-slate-400">{t('settings_import_desc')}</p>
            </div>
            <ChevronRight size={16} className="text-slate-300" />
          </button>
        ...
        <div className={sectionClass}>
          <div className={rowClass}>
            <div className="w-9 h-9 rounded-xl bg-slate-100 flex items-center justify-center"><Info size={16} className="text-slate-400" /></div>
            <div className="flex-1">
              <p className="text-[13px] font-semibold text-slate-700">{t('settings_about')}</p>
