...
export function ActivityPage() {
  ...
  return (
    <main className="min-h-dvh bg-cream-bg pb-28">
      ...
      <div className="sukoon-body min-h-[60dvh] px-5 pt-5 space-y-5">
        {!hasAnyItems ? (
          ...
          <>
            <section>
              <div className="flex items-center justify-between mb-2.5">
                <p className="text-[11px] font-bold text-ink-500 uppercase tracking-widest">Shared Updates</p>
                {unreadCount > 0 && (
              ...
              {notifications.length === 0 ? (
                <div className="rounded-2xl bg-cream-card border border-cream-border p-4 text-[12px] text-ink-500">No shared notifications yet.</div>
              ) : (
                <div className="space-y-2">
                  {notifications.map((notification, index) => (
                    <div
                      key={notification.id}
                      className={`rounded-2xl bg-cream-card border border-cream-border !rounded-2xl p-4 flex items-start gap-3 animate-fade-in border-l-4 ${
                        notification.readAt ? 'border-l-slate-200' : 'border-l-amber-400'
                      }`}
                      style={{ animationDelay: `${index * 30}ms` }}
                    >
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 mt-0.5 ${
                        notification.readAt ? 'bg-cream-soft text-ink-500' : 'bg-amber-50 text-amber-600'
                      }`}>
                        <BellRing size={15} strokeWidth={1.8} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-[13px] font-semibold text-ink-800 leading-snug tracking-tight">{notification.title}</p>
                        <p className="text-[12px] text-ink-500 mt-1">{notification.body}</p>
                        <p className="text-[10px] text-ink-500 mt-1">{format(new Date(notification.createdAt), 'dd MMM, h:mm a')}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <section>
              <p className="text-[11px] font-bold text-ink-500 uppercase tracking-widest mb-2.5">Personal Activity</p>
              {activities.length === 0 ? (
                <div className="rounded-2xl bg-cream-card border border-cream-border p-4 text-[12px] text-ink-500">No personal activity yet.</div>
              ) : (
                <div className="space-y-6">
                  {Object.entries(grouped).map(([dateLabel, items]) => (
                    <div key={dateLabel}>
                      <p className="text-[11px] font-bold text-ink-500 uppercase tracking-widest mb-2.5">{dateLabel}</p>
                      <div className="space-y-2">
                        {items.map((activity, index) => {
                          ...
                          return (
                            <div
                              key={activity.id}
                              className={`rounded-2xl bg-cream-card border border-cream-border !rounded-2xl p-4 flex items-start gap-3 animate-fade-in ${style.card}`}
                              style={{ animationDelay: `${index * 30}ms` }}
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="text-[13px] text-ink-800 leading-snug tracking-tight">{activity.description}</p>
                                <p className="text-[10px] text-ink-500 mt-1">{format(new Date(activity.timestamp), 'h:mm a')}</p>
                              </div>
                            </div>
                          );
                        })}
