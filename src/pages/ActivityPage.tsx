...
export function ActivityPage() {
  ...
  const grouped = activities.reduce((acc, a) => {
    const label = getDateLabel(a.timestamp);
            ...
            {Object.entries(grouped).map(([dateLabel, items]) => (
              <div key={dateLabel}>
                ...
                <div className="space-y-2">
                  {items.map((a, i) => {
                    const Icon = iconMap[a.type] ?? ArrowLeftRight;
                    const style = styleMap[a.type] ?? defaultStyle;
                    return (
                      <div key={a.id}
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-[13px] text-slate-700 leading-snug tracking-tight">{a.description}</p>
                          <p className="text-[10px] text-slate-400 mt-1">{format(new Date(a.timestamp), 'h:mm a')}</p>
                        </div>
                      </div>
                    );
                  })}
